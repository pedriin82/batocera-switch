...
#                   ----------------------------------------                   #
#                     > github.com/pedriin82/batocera-switch                   #
#                                                                              #     
################################################################################
#  ---------------
#     SETTINGS 
...
EMULATORS="$(echo $EMULATORS | sed 's/ /-/g')"
   ...
   if [[ ! -f $cfg ]]; then 
      link_defaultconfig=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-config.txt
...
# \\ 
link_tar=https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/batocera-switch-tar
link_libselinux=https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/batocera-switch-libselinux.so.1
...
# fix broken libstdc++.so.6 for v37 
		  if [[ "$(uname -a | awk '{print $3}')" > "6.2" ]]; then 
			 link_libstdc=https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/batocera-switch-libstdc++.so.6
...
# else 
    # wget -q --show-progress --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/appimages/ryujinx-${version}-linux_x64.tar.gz" "https://foclabroc.freeboxos.fr:55973/share/vSOXdZCzArl0r-GL/ryujinx-canary-1.2.448-linux_x64.tar.gz"
...
# if [ -f "$link_ryujinx" ]; then	
    # if [ -f "$link_ryujinx" ] && [ $(stat -c%s "$link_ryujinx") -gt 2048 ]; then
          ...
          # else
	       # echo -e "${T}RYUJINX   [${W}!!${T}] download fail put ryujinx-1.2.448-linux_x64.tar.gz in (/system/switch/appimages) then relaunch script"    	
...
link_ryujinx=/userdata/system/switch/ryujinx-${version}-linux_x64.tar.gz
link_tarR="/userdata/system/switch/appimages/ryujinx-canary-1.3.138-linux_x64.tar.gz"
# --------------------------------------------------------
if [ "$N" = "1" ]; then C=""; else C="$E/$N"; fi
if [ -f "$link_ryujinx" ]; then
    if [ -f "$link_ryujinx" ] && [ $(stat -c%s "$link_ryujinx") -gt 2048 ]; then
...
# /userdata/system/switch/extra/ryujinx/ will keep all ryujinx related dependencies
		  ...
		  wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O "$extra/$emu/xdg-mime" "https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/xdg-mime"
		  # ###curl -sSf "https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/xdg-mime" -o "$extra/$emu/xdg-mime"
		  ...
		  #wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/extra/libSDL2.so" "https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/libSDL2.so"
...
mkdir /userdata/system/switch/appimages 2>/dev/null
# cd /userdata/system/switch/appimages
# citronE="/userdata/system/switch/appimages/citron0.9.0.AppImage"
# if [ -f "$citronE" ]; then
    # cp /userdata/system/switch/appimages/citron0.9.0.AppImage /userdata/system/switch/citron.AppImage 2>/dev/null;
# else 
    # wget -q --show-progress --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/appimages/citron0.9.0.AppImage" "https://foclabroc.freeboxos.fr:55973/share/7mG-MyEjz3uO9Vqq/citron0.9.0.AppImage"
    # cp /userdata/system/switch/appimages/citron0.9.0.AppImage /userdata/system/switch/citron.AppImage 2>/dev/null; fi
        # if [ -f "$citronE" ] && [ $(stat -c%s "$citronE") -gt 2048 ]; then
		   # echo -e "${T}CITRON   ${T}❯❯   ${T}/V0.9.0/ ${GREEN}SUCCESS";
		# else
		   # rm /userdata/system/switch/appimages/citron0.9.0.AppImage 2>/dev/null
		   # echo -e "${T}CITRON   [${W}!!${T}] download fail put citron0.9.0.AppImage in (/system/switch/appimages) then relaunch script"; fi
...
if [ -f "$link_citron" ]; then
    if [ -f "$link_citron" ] && [ $(stat -c%s "$link_citron") -gt 2048 ]; then
...
# ------------------------------------------------------------------- 
   extraurl="https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra"
# ------------------------------------------------------------------- 
...
#   wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/configgen/mapping.csv" "https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/configgen/mapping.csv"
...
# -------------------------------------------------------------------
# prepare patcher 
url_patcher="https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-patcher.sh"
...
url_switchkeys=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/configs/evmapy/switch.keys
url_es_features_switch=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/configs/emulationstation/es_features_switch.cfg
url_es_systems_switch=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/configs/emulationstation/es_systems_switch.cfg
url_switchlauncher=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/configgen/switchlauncher.py
url_GeneratorImporter=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/configgen/GeneratorImporter.py
url_ryujinxMainlineGenerator=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/configgen/generators/ryujinx/ryujinxMainlineGenerator.py
url_yuzuMainlineGenerator=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/configgen/generators/yuzu/yuzuMainlineGenerator.py
url_sshupdater=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-sshupdater.sh
url_updater=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-updater.sh
url_portsupdater=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/roms/ports/Switch%20Updater.sh
url_portsupdaterkeys=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/roms/ports/Switch%20Updater.sh.keys   
url_edendesktop=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-config-eden
...
# additional pass for folks who have issues connecting to github
   function get() {
      file="$1"
      path=/userdata/system/switch/configgen/sdl2
      url=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/configgen/sdl2
         mkdir -p $path 2>/dev/null
            if [[ ! -e "$path/$file" ]]; then
               cd $path
...
sdlurl=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-libSDL2.so
   if [[ ! -e "$sdl" ]]; then 
...
url=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/translations
mkdir -p $path 2>/dev/null
...
# GET TRANSLATOR
translator=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-translator.sh
...
file=https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-ryujinx-fixes.sh
...
# include display output: 
   ...
   mkdir /userdata/system/switch 2>/dev/null; mkdir /userdata/system/switch/extra 2>/dev/null
      if [[ ( -e "$tput" && "$(wc -c "$tput" | awk '{print $1}')" < "444" ) || ( ! -e "$tput" ) ]]; then
         rm "$tput" 2>/dev/null
         wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O /userdata/system/switch/extra/batocera-switch-tput https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/batocera-switch-tput
         ##curl -sSf "https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-tput" -o "/userdata/system/switch/extra/batocera-switch-tput"
      fi
      if [[ ( -e "$libtinfo" && "$(wc -c "$libtinfo" | awk '{print $1}')" < "444" ) || ( ! -e "$libtinfo" ) ]]; then
         rm "$libtinfo" 2>/dev/null
         wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O /userdata/system/switch/extra/batocera-switch-libtinfo.so.6 https://github.com/pedriin82/batocera-switch/raw/main/system/switch/extra/batocera-switch-libtinfo.so.6
         ##curl -sSf "https://raw.githubusercontent.com/pedriin82/batocera-switch/main/system/switch/extra/batocera-switch-libtinfo.so.6" -o "/userdata/system/switch/extra/batocera-switch-libtinfo.so.6"
      fi
